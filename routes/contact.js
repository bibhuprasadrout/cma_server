...
//fetch contacts
contactRouter.get("/mycontacts", auth, async (req, res) => {
  try {
...
//update a contact
contactRouter.put("/contact", auth, async (req, res) => {
  const { id } = req.body;
  if (!id)
    return res.status(400).json({ error: "No conatct id is specified." });
...
//delete a contact
contactRouter.delete("/delete/:id", auth, async (req, res) => {
  const { id } = req.params;
  if (!id)
    return res.status(400).json({ error: "No conatct id is specified." });
  if (!mongoose.isValidObjectId(id))
...
contactRouter.get("/contact/:id", auth, async (req, res) => {
  const { id } = req.params;
